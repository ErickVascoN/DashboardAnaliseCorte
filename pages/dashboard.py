...
from config import METAS_PADRAO

# Verificar se empresa foi selecionada
if 'empresa_config' not in st.session_state:
    st.error("❌ Nenhuma empresa selecionada. Volta para a página inicial.")
    st.stop()
...
# =====================================================================
empresa_config = st.session_state.empresa_config
st.set_page_config(
    page_title=f"Dashboard - {empresa_config['nome_display']}",
    page_icon="✂️",
    layout="wide",
...
# =====================================================================
GOOGLE_SHEETS_ID = empresa_config['sheet_id']
GOOGLE_SHEETS_GID = empresa_config['gid']
...
# Usar metas padrão por enquanto (futuro: ler de aba METAS da planilha)
METAS = METAS_PADRAO
...
# HEADER
# =====================================================================
st.markdown(f'<div class="main-header">{empresa_config["emoji"]} Dashboard Controle de Corte - {empresa_config["nome_display"]}</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    estacoes = sorted(estacoes_disponiveis)
    
    # Gerar cores dinamicamente para estações
    cores_estacao = {}
    cores_paleta = ['#1f77b4', '#2ca02c', '#ff7f0e', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f']
    for i, est in enumerate(estacoes):
        cores_estacao[est] = cores_paleta[i % len(cores_paleta)]
    ...
    st.markdown("#### 📈 Produção Diária por Estação")
    ...
    for est_nome in estacoes:
        meta_val = METAS.get(est_nome, 0)
        if meta_val > 0:
...
st.markdown(
    ...
    f"✂️ Dashboard Controle de Corte - {empresa_config['nome_display']} | "
